...
function get_ket($suhu,$nilai){
	$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='$suhu' and nilai='".$nilai."' GROUP BY is_sub_golongan,keterangan_nilai");
...
function nilai_parameter_a($is_sub_golongan,$keterangan,$suhu,$keterangan_nilai,$golongan,$ket_nilai){
	$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='$golongan' and nilai='".$suhu."' and keterangan_nilai='".$keterangan_nilai."' GROUP BY is_sub_golongan,keterangan_nilai");
					while($row=mysql_fetch_array($sql)){
		...
		<div class="panel panel-default">
				...
				<div class="container">
				  ...
				  <div class="tab-content">
					...
					<table class="table table-bordered" style="width:300px">
						...
						<div id="chartContainer" style="height: 370px;width:800px;"></div>
								...
								<table class='table table-bordered'>
									...
									<?php
									mysql_query("delete from tbl_hasil");
									mysql_query("delete from tbl_hasil_2");
									$suhu=$_POST['suhu'];
									$nomor=1;
									$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='0' and nilai='".$_POST['suhu']."' GROUP BY is_sub_golongan,keterangan_nilai");
									while($row=mysql_fetch_array($sql)){
										$sql2=mysql_query("SELECT MIN( nilai ) AS nilai_bawah, MAX( nilai ) AS nilai_atas
															FROM  `tbl_rule` 
															WHERE is_golongan =  '0'
					...
					mysql_query("INSERT INTO tbl_hasil(urutan,golongan,nilai,keterangan,hasil)VALUES('$nomor','0','".$_POST['suhu']."','".$row['keterangan_nilai']."','".number_format($hasil,2)."')");
				...
				<table class="table table-bordered" style="width:900px">
					...
					<?php
						$nomor=1;
					$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='0' GROUP BY is_sub_golongan,keterangan_nilai ORDER BY id_golongan ASC");
					while($row=mysql_fetch_array($sql)){
						if($row['is_sub_golongan'] == 1){
...
<!-- PERHITUNGAN UNTUK SOIL MOISTURE -------------------------------------------------------------------------------------------->
			...
			<div id="chartContainer2" style="height: 370px;width:800px;"></div>
				...
				<table class='table table-bordered'>
					...
					$nomor=1;
					$suhu=$_POST['sm'];
					$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='1' and nilai='".$_POST['sm']."' GROUP BY is_sub_golongan,keterangan_nilai ORDER BY id_golongan ASC");
					while($row=mysql_fetch_array($sql)){
						$sql2=mysql_query("SELECT MIN( nilai ) AS nilai_bawah, MAX( nilai ) AS nilai_atas
											FROM  `tbl_rule` 
											WHERE is_golongan =  '1'
					...
					mysql_query("INSERT INTO tbl_hasil(urutan,golongan,nilai,keterangan,hasil)VALUES('$nomor','1','".$_POST['sm']."','".$row['keterangan_nilai']."','".number_format($hasil,2)."')");
				...
				<table class="table table-bordered" style="width:900px">
					...
					<?php
						$nomor=1;
					$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='1' GROUP BY is_sub_golongan,keterangan_nilai ORDER BY id_golongan ASC");
					while($row=mysql_fetch_array($sql)){
						if($row['is_sub_golongan'] == 1){
...
<!-- PERHITUNGAN UNTUK HUMIDITY -------------------------------------------------------------------------------------------->
				...
				<table class='table table-bordered'>
					...
					$nomor=1;
					$suhu=$_POST['hm'];
					$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='2' and nilai='".$_POST['hm']."' GROUP BY is_sub_golongan,keterangan_nilai ORDER BY id_golongan ASC");
					while($row=mysql_fetch_array($sql)){
						$sql2=mysql_query("SELECT MIN( nilai ) AS nilai_bawah, MAX( nilai ) AS nilai_atas
											FROM  `tbl_rule` 
											WHERE is_golongan =  '2'
					...
					mysql_query("INSERT INTO tbl_hasil(urutan,golongan,nilai,keterangan,hasil)VALUES('$nomor','2','".$_POST['hm']."','".$row['keterangan_nilai']."','".number_format($hasil,2)."')");
				...
				<table class="table table-bordered" style="width:900px">
					...
					<?php
						$nomor=1;
					$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='2' GROUP BY is_sub_golongan,keterangan_nilai ORDER BY id_golongan ASC");
					while($row=mysql_fetch_array($sql)){
						if($row['is_sub_golongan'] == 1){
												$nama_golongan="Trapezoid";
													$nilai_b="D";
...
<div style="height:60px;width:800px;background-color:white;z-index: 99;margin-top:30px; position: absolute"></div>
				...
				<table class='table table-bordered'>
					...
					$nomor=1;
					$suhu=$_POST['li'];
					$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='3' and nilai='".$_POST['li']."' GROUP BY is_sub_golongan,keterangan_nilai ORDER BY id_golongan ASC");
					while($row=mysql_fetch_array($sql)){
						$sql2=mysql_query("SELECT MIN( nilai ) AS nilai_bawah, MAX( nilai ) AS nilai_atas
											FROM  `tbl_rule` 
											WHERE is_golongan =  '3'
					...
					mysql_query("INSERT INTO tbl_hasil(urutan,golongan,nilai,keterangan,hasil)VALUES('$nomor','3','".$_POST['li']."','".$row['keterangan_nilai']."','".number_format($hasil,2)."')");
				...
				<table class="table table-bordered" style="width:900px">
					...
					<?php
						$nomor=1;
					$sql=mysql_query("SELECT * FROM `tbl_rule` WHERE is_golongan='3' GROUP BY is_sub_golongan,keterangan_nilai ORDER BY id_golongan ASC");
					while($row=mysql_fetch_array($sql)){
						if($row['is_sub_golongan'] == 1){
												$nama_golongan="Trapezoid";
													$nilai_b="D";
...
<!-- PERHITUNGAN UNTUK FUZYFICATION -------------------------------------------------------------------------------------------->
					...
					<table class='table table-bordered' style="width:930px">
						...
						function get_rule_kondisi($kondisi_1,$kondisi_2,$kondisi_3){
							$sql=mysql_query("SELECT * from tbl_rule_kondisi where kondisi_1 LIKE '%$kondisi_1%' and kondisi_2 LIKE '".$kondisi_2."'  and kondisi_3 LIKE '".$kondisi_3."' ");
								while($row=mysql_fetch_array($sql)){
									$kondisi=$row['hasil'];
								}
								return $kondisi;
						}
							$sql=mysql_query("SELECT * from tbl_hasil where golongan='0' ");
								while($row=mysql_fetch_array($sql)){
									...
									";
									$sql2=mysql_query("SELECT * from tbl_hasil where golongan='1' ");
								while($row_6=mysql_fetch_array($sql2)){
									$sql3=mysql_query("SELECT * from tbl_hasil where golongan='2' ");
										while($row_7=mysql_fetch_array($sql3)){
												$sql4=mysql_query("SELECT * from tbl_hasil where golongan='3' ");
						...
						$ra=0;
							...
							$sql=mysql_query("SELECT * from tbl_hasil where golongan='0' ");
								while($row=mysql_fetch_array($sql)){
									...
									$sql2=mysql_query("SELECT * from tbl_hasil where golongan='1' ");
								while($row_6=mysql_fetch_array($sql2)){
									$sql22=mysql_query("SELECT * from tbl_hasil where golongan='2' ");
									...
									while($row_7=mysql_fetch_array($sql22)){
										$sql4=mysql_query("SELECT * from tbl_hasil where golongan='3' ");
												...
												$ket=$row_8['keterangan'];
														...
														mysql_query("INSERT INTO tbl_hasil_2(urutan,golongan,nilai,keterangan,hasil)VALUES('$nomor','".get_rule_kondisi($row['keterangan'],$row_6['keterangan'],$row_7['keterangan'],$row_8['keterangan'])."','".$_POST['suhu']."','".$row['keterangan']."','".MIN($row['hasil'],$row_6['hasil'],$row_7['hasil'],$row_8['hasil'])."')");
													}
									}
								}
...
<!-- PERHITUNGAN UNTUK FUZYFICATION -------------------------------------------------------------------------------------------->
					...
					<table class='table table-bordered' style="width:930px">
						...
						function get_duration($kondisi_1,$nilai_set){
							$nilai=0;
							$sql=mysql_query("SELECT * from tbl_rule_duration where keterangan='".$kondisi_1."' LIMIT 0,4");
								while($row=mysql_fetch_array($sql)){
						...
						$nomor=1;
							$total=0;
							$total_bagi=0;
							$sql=mysql_query("SELECT golongan,MAX(hasil)as total_max from tbl_hasil_2 GROUP BY golongan ");
								while($row=mysql_fetch_array($sql)){
...
<!-- PERHITUNGAN UNTUK DEFUZYFICATION -------------------------------------------------------------------------------------------->
					...
					<?php
							...
							mysql_query("UPDATE tbl_hasil_menit SET tanggal='".$_POST['tanggal']."',suhu='".$_POST['suhu']."',sm='".$_POST['sm']."',humidity='".$_POST['hm']."',li='".$_POST['li']."',nilai='".round($hasil_meni)."' where id_hasil='".$_POST['id_hasil']."'");
